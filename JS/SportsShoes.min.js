(() => {
    var t = window.location.href,
        e = new URL(t).searchParams.get('page');
    let n = 'list-products',
        o = 'sitemap',
        a = 'introduce',
        s = 'category',
        i = 'filter',
        c = '.header__navbar__item--';
    var l;
    e
        ? ((document.title = (l = e).charAt(0).toUpperCase() + l.slice(1)),
          (n += `-${e}`),
          (o += `-${e}`),
          (a += `-${e}`),
          (s += `-${e}`),
    ...
    const r = [n, o, a, s, i];
    Promise.all(
        r.map((t) => fetch(`https://comfortable-fox-gear.cyclic.app/${t}`)),
    )
        .then((t) => Promise.all(t.map((t) => t.text())))
        .then((t) => {
            (t[1] = JSON.parse(t[1])),
                $('section').prepend(
                    `\n            <nav class="sitemap" style="background: url(${
                        t[1].background
                    });" data-sitemap-item="${t[1].item.join(
                        ', ',
                    )}" data-sitemap-link="${t[1].link.join(
                        ', ',
                    )}" data-sitemap-title="${t[1].title}"></nav>\n        `,
                ),
                Sitemap(),
                $('.sub-sitemap').html(
                    `\n            <img class="img-fluid" src="${t[1].background}" alt="${t[1].title}">\n            <h1 class="sitemap__title mt-4 mb-0">${t[1].title}</h1>\n        `,
                );
            const e = JSON.parse(t[2]);
            $('.content .introduce').html(e.description),
                e['list-products'] &&
            ...
            handleSportsShoes(JSON.parse(t[0]), o, a, s, i, l, r, d),
                window.innerWidth >= 991 && $(c)?.addClass('active'),
                $('.loading').addClass('hidden'),
                $('.app').removeClass('hidden');
        });
})();
const getPriceProduct = (t) =>
    t.discount ? (t.price * (100 - t.discount)) / 100 : t.price;
function handleSportsShoes(t, e, n, o, a, s, i, c) {
    const l = $('.category__icon'),
        r = $('.category__primary__body'),
        d = $('.category__product__body--on-sale'),
        _ = $('.category__product__body--best-sellers'),
        p = $('.category__product__body--new-products'),
        m = $('.content-product__filter__sort__by'),
        g = $('.content-product__list'),
        u = $('.content-product__filter__icon__item');
    l.click(function () {
        $(this).toggleClass('active'),
            $(this).hasClass('active')
                ? $(this).next('.category__submenu').animateAuto('height', 200)
                : $(this)
                      .next('.category__submenu')
            ...
            .map(
                (e) =>
                    `\n                        <li class="category__item">\n                            <input id="${
                        ...
                        e.name
                    }" type="checkbox" name="${t.toLowerCase()}">\n                            <label for="${
                        e.name
                    }">${e.name} <span>(${
                        e.number
                    })</span></label>\n                        </li>\n                    `,
    ...
    e &&
        ...
        r.append(h('Compositions', a)),
        r.append(h('Styles', s)),
        r.append(h('Properties', i)),
        r.append(h('Price', c)));
    [...$('.category__item__color')].forEach((t) => {
        $(t)
            .children('.category__item__color__checkbox')
            .css('background-color', t.getAttribute('data-bg-color'));
    });
    const v = (t) =>
    ...
    d.html(
        ...
        m.click(function (t) {
            $(this).toggleClass('active'), t.stopPropagation();
        }),
        app.click(() => m.removeClass('active'));
    let f = {};
    const y = (t, e) => {
        let n = [...t];
        const o = +g[0].getAttribute('data-page'),
            a = g.attr('data-filter').split('|');
        a[0]
            ? n.sort((t, e) =>
                  ('price' === a[0] &&
                      getPriceProduct(t) !== getPriceProduct(e)) ||
                  t[a[0]] === e[a[0]]
                      ? getPriceProduct(t) > getPriceProduct(e)
                          ? a[1]
                          : -1 * a[1]
            ...
            ((t) => {
                const e = t.length;
                $('.content-product__filter__number span').html(e),
                    $('.content-product__pagination__title .count').html(e),
            ...
            $('.content-product__pagination__btn-nav')[
                n.length <= 9 ? 'addClass' : 'removeClass'
            ]('hidden');
        const s = JSON.parse(localStorage.getItem('TTTHH-COMPARE-KEY')) ?? [];
    ...
    $('.content-product__filter__number span').html(t.length),
        $('.content-product__pagination__title .count').html(t.length),
        g.html(y(t)),
    ...
    const b = $('.content-product__pagination__btn');
    b.click(function () {
        let e;
        if (
            (b.removeClass('hidden'),
            b.removeClass('active'),
            $(this).hasClass('content-product__pagination__btn--first'))
        )
            (e = 1), $(b[0]).addClass('hidden'), $(b[1]).addClass('active');
        else if ($(this).hasClass('content-product__pagination__btn--last')) {
            e = Math.ceil(t.length / 9);
            let n = b.length;
            $(b[n - 1]).addClass('hidden'), $(b[n - 2]).addClass('active');
        } else
            (e = +g[0].getAttribute('data-page') + 1),
                $(this).addClass('active');
        g[0].setAttribute('data-page', e),
            g.html(y(t)),
            $('.content-product__pagination__title .from').html(
                9 * (e - 1) + 1,
            ),
            $('.content-product__pagination__title .to').html(
                Math.min(t.length, 9 * e),
            ),
            scrollToElement($('.content-product')[0], 'start');
    }),
        u.click(function (t) {
            u.removeClass('active'), $(this).addClass('active');
            const e = $('.content-product__list > .row > div').children(),
                n = e.children();
    ...
    const S = $('.content-product__filter__sort__by__name span.name');
    [...$('.content-product__filter__sort__by__list ul li')].forEach((e) => {
        $(e).click(function () {
            S.html(this.innerHTML),
                g.attr('data-filter', this.getAttribute('data-filter')),
                g.attr('data-page', 1),
                b.removeClass('hidden'),
                b.removeClass('active'),
                $(b[0]).addClass('hidden'),
        ...
        $('.category__primary__body .category__item input').change(function () {
            ...
            this.checked
                ? f[e]
                    ? f[e].push(n)
                    : (f[e] = [n])
                : (f[e] = f[e].filter((t) => t !== n)),
                g[0].setAttribute('data-page', 1),
                g.html(y(t, f));
        }),
        $('.category__primary__body .category__item__color').click(function () {
            const e = this.getAttribute('data-bg-color');
            $(this).toggleClass('active'),
                $(this).hasClass('active')
                    ? f.color
                        ? f.color.push(e)
                        : (f.color = [e])
                    : (f.color = f.color?.filter((t) => t !== e)),
                g[0].setAttribute('data-page', 1),
                g.html(y(t, f));
        }),
        $('.category__primary__body .clear-all span').click(() => {
            [
                ...
                $(
                    '.category__primary__body .category__item__color',
                ).removeClass('active'),
                (f = {}),
                g.html(y(t));
        }),
