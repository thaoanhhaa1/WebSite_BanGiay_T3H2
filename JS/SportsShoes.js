(() => {
        ...
        <ul class="category__list">
            ${category['category-list']
                ...
                <li class="category__item">
                    <a href="${
                        categoryItem.link || ''
                    }" class="category__link">${categoryItem.title} (<span>${
                        categoryItem.number
                    }</span>)</a>
                    <div class="category__icon ${
                        categoryItem.submenu || 'hidden'
                    }">
    ...
    Promise.all(
        ...
        .then((texts) => {
            ...
            $('section').prepend(`
            <nav class="sitemap" style="background: url(${
                texts[1].background
            });" data-sitemap-item="${texts[1].item.join(
                ', ',
            )}" data-sitemap-link="${texts[1].link.join(
                ', ',
            )}" data-sitemap-title="${texts[1].title}"></nav>
        `);
            Sitemap();
        ...
        `);
            ...
            if (introduction['list-products'])
                $('.product-introduction').html(
                    getHTMLIntroduce(introduction['list-products']),
                );

            const categoryDanger = JSON.parse(texts[3]);
            if (categoryDanger.title)
                $('.category__danger').html(
                    getHTMLCategoryDanger(categoryDanger),
                );
            ...
            if (window.innerWidth >= 991) $(active)?.addClass('active');
...
function handleSportsShoes(
    ...
    const categoryProductBestSellers = $(
        '.category__product__body--best-sellers',
    );
    const categoryProductNewProducts = $(
        '.category__product__body--new-products',
    );
    const contentProductFilterSortBy = $('.content-product__filter__sort__by');
    const contentProductListElement = $('.content-product__list');
    const contentProductFilterIconItem = $(
        '.content-product__filter__icon__item',
    );
            ...
            <ul class="category__list category__list--checkbox">
                ${products
                    .map((product) => {
                        return `
                        <li class="category__item">
                            <input id="${product.name}" value="${
                            product.name
                        }" type="checkbox" name="${title.toLowerCase()}">
                            <label for="${product.name}">${
                            product.name
                        } <span>(${product.number})</span></label>
                        </li>
                    `;
                    })
                    .join('')}
    ...
    if (Categories) {
        ...
        categoryPrimaryBody.append(
            getHTMLCheckbox('Compositions', Compositions),
        );
    ...
    [...categoryItemColor].forEach((item) => {
        $(item)
            .children('.category__item__color__checkbox')
            .css('background-color', item.getAttribute('data-bg-color'));
    });
                ...
                ${listProduct
                    ...
                    <li class="category__product__item">
                        <div class="category__product__img">
                            <a href="">
                                <img class="img-cover " src="./IMG/${
                                    item.image
                                }" alt="${item.name}">
                                <img class="img-cover hover" src="./IMG/${
                                    item.imageHover
                                }" alt="${item.name}">
                            </a>
                        </div>
    ...
    const updateNumberContentProduct = (products) => {
        ...
        $('.content-product__pagination__title .from').html(
            length === 0 ? length : (indexPage - 1) * 9 + 1,
        );
        $('.content-product__pagination__title .to').html(
            Math.min(length, indexPage * 9),
        );
    ...
    const getHTMLContentProductList = (productList, filters) => {
        ...
        const dataFilter = contentProductListElement
            .attr('data-filter')
            .split('|');
            ...
            products.sort((a, b) => {
                if (dataFilter[0] === 'price')
                    if (getPriceProduct(a) !== getPriceProduct(b))
                        return getPriceProduct(a) > getPriceProduct(b)
                            ? dataFilter[1]
                            : -1 * dataFilter[1];
                if (a[dataFilter[0]] === b[dataFilter[0]])
                    return getPriceProduct(a) > getPriceProduct(b)
                        ? dataFilter[1]
                        : -1 * dataFilter[1];
                return a[dataFilter[0]] > b[dataFilter[0]]
                    ? dataFilter[1]
                    : -1 * dataFilter[1];
            });
        ...
        $('.content-product__pagination__btn-nav')[
            products.length <= 9 ? 'addClass' : 'removeClass'
        ]('hidden');
                        ...
                        <div class="content-product__item pb-md-3 position-relative d-flex flex-column no-gutters">
                            ...
                            <div class="content-product__item__info order-3">
                                ...
                                <h4 class="content-product__item__info__name">
                                  ...
                                  product.slug
                              }">${product.name.replace('$$', "'")}</a>
                                </h4>
                                <p class="content-product__item__info__price">
                                    <span class="mx-0 old-price ${
                            ...
                            <ul class="function-buttons list-unstyled order-2 mt-3-md mb-0-md">
                                <li class="hidden-sm" onclick='showModalProduct(${JSON.stringify(
                                    product,
                                )})' style="--i: 0;"><a class="d-flex align-items-center justify-content-center"><i class="fas fa-expand-arrows-alt"></i></a></li>
                                <li class="${
                                    compareProductList.hasProduct(product) &&
                                    'active'
                                }" onclick='handlerClickCompare(this, ${JSON.stringify(
                                  product,
                              )}, "#modal-tost", "The product has been added to list compare. <a href=${`/ProductsCompare.html`}>View list compare.</a>")' style="--i: 1;"><a class="d-flex align-items-center justify-content-center"><i class="fas fa-redo"></i></a></li>
                                <li onclick='handlerClickCart(this, ${JSON.stringify(
                                    product,
                                )}, "#modal-tost", "${
                                  product.name
                              } Product successfully added to your shopping cart. <a href=${`/cartPage.html`}>View cart.</a>")' style="--i: 2;"><a class="d-flex align-items-center justify-content-center"><i class="fas fa-shopping-cart"></i></a></li>
                            </ul>
                        </div>
                    </div>
                `,
    ...
    $('.content-product__pagination__title .count').html(
        contentProductList.length,
    );
    contentProductListElement.html(
        getHTMLContentProductList(contentProductList),
    );
    $('.content-product__pagination__btn-nav').html(
        getHTMLContentProductPagination(
            Math.ceil(contentProductList.length / 9),
        ),
    );
    $('.content-product__pagination__title .from').html(
        Math.min(1, contentProductList.length),
    );
    $('.content-product__pagination__title .to').html(
        Math.min(9, contentProductList.length),
    );
    ...
    contentProductPaginationBtn.click(function () {
        ...
        if ($(this).hasClass('content-product__pagination__btn--first')) {
            ...
            $(contentProductPaginationBtn[lengthPaginationBtn - 1]).addClass(
                'hidden',
            );
            $(contentProductPaginationBtn[lengthPaginationBtn - 2]).addClass(
                'active',
            );
        } else {
            indexPage =
                +contentProductListElement[0].getAttribute('data-page') + 1;
        ...
        $('.content-product__pagination__title .to').html(
            Math.min(contentProductList.length, indexPage * 9),
        );
        scrollToElement($('.content-product')[0], 'start');
    });
    ...
    contentProductFilterIconItem.click(function (e) {
        ...
        const containerItem = $(
            '.content-product__list > .row > div',
        ).children();
        const childrenElement = containerItem.children();

        if (
            $('.content-product__filter__icon__item--list').hasClass('active')
        ) {
            $('.content-product__list > .row > div').removeClass();
            $('.content-product__list > .row > div').addClass(
            ...
            [...childrenElement].forEach((element, index) => {
                if (index % 3 === 0) {
                    ...
                    $(element).addClass(
                        ' col col-xl-4 col-lg-4 col-md-6 col-sm-6 col-6 mb-0',
                    );
                } else if (index % 3 === 1) {
                    $(element).removeClass('order-3');
                    $(element).addClass(
                        ' col col-xl-8 col-lg-8 col-md-6 col-sm-6 col-6',
                    );
                } else {
            ...
            $('.content-product__list > .row > div').removeClass();
            $('.content-product__list > .row > div').addClass(
                'col col-xl-4 col-lg-4 col-md-6 col-sm-6 col-6',
            );
    ...
    const filterProductList = $(
        '.content-product__filter__sort__by__list ul li',
    );

    [...filterProductList].forEach((filterItem) => {
        $(filterItem).click(function () {
            filterLabel.html(this.innerHTML);
            contentProductListElement.attr(
                'data-filter',
            ...
            $('.content-product__pagination__title .to').html(
                Math.min(contentProductList.length, 9),
            );
            scrollToElement($('.content-product')[0], 'start');
        });
    });
    ...
    $('.category__primary__body .category__item input').change(function () {
        ...
        if (this.checked) {
            ...
            filtersProductList[name] = filtersProductList[name].filter(
                (item) => item !== value,
            );
        }
        contentProductListElement[0].setAttribute('data-page', 1);
        contentProductListElement.html(
            getHTMLContentProductList(contentProductList, filtersProductList),
        );
    });
    ...
    $('.category__primary__body .category__item__color').click(function () {
        ...
        } else filtersProductList['color'] = filtersProductList['color']?.filter((item) => item !== value);
        contentProductListElement[0].setAttribute('data-page', 1);
        contentProductListElement.html(
            getHTMLContentProductList(contentProductList, filtersProductList),
        );
    ...
    $('.category__primary__body .clear-all span').click(() => {
        [
            ...$('.category__primary__body .category__item input:checked'),
        ].forEach((item) => (item.checked = false));
        $('.category__primary__body .category__item__color').removeClass(
            'active',
        );
        filtersProductList = {};
        contentProductListElement.html(
